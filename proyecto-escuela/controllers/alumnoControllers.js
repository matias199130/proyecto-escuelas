...
exports.crearAlumno = (req, res) => {
    ...
    connection.query('INSERT INTO alumnos (nombre, apellido, edad, curso) VALUES (?, ?, ?, ?)', [nombre, apellido, edad, curso], (error, results) => {
      if (error) {
...
exports.obtenerAlumnoPorId = (req, res) => {
    ...
    connection.query('SELECT * FROM alumnos WHERE id = ?', [id], (error, results) => {
      if (error) {
...
exports.actualizarAlumno = (req, res) => {
    const id = req.params.id;
    const { nombre, apellido, edad, curso } = req.body;
    connection.query('UPDATE alumnos SET nombre = ?, apellido = ?, edad = ?, curso = ? WHERE id = ?', [nombre, apellido, edad, curso, id], (error, results) => {
      if (error) {
